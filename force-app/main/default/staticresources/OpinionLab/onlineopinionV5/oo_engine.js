  ...
  __detectBrowser: function(ua){
    var
      ...
      browser = {
        ...
        WindowsPhone: ua.toLowerCase().indexOf('windows phone') > -1,
        WindowsTablet: ua.toLowerCase().indexOf('windows nt') > -1 && ua.toLowerCase().indexOf('touch') > -1,
    ...
    browser.isMobile = (browser.MobileSafari || browser.BlackBerry || browser.Fennec || browser.IEMobile || browser.OperaMobile || browser.Kindle || browser.iOs || browser.Android || browser.WindowsPhone || browser.WindowsTablet);
    browser.isMobileNonIOS = (browser.isMobile && !browser.iOs);
    return browser;
  }
};
...
(function () {
  ...
  function showWaypoint(markup) {
    ...
    OOo.addEventListener(closePrompt, 'click', function (e) { var evt = e || window.event; OOo.hideWaypoint(evt, lastFocused); }, false);
  ...
  function getPrompt() {
    var markup = "<div id='oo_invitation_prompt' role='dialog' aria-describedby='oo_invite_message'><div id='oo_company_logo'></div><div id='oo_invite_content'><p id='oo_invite_message'>After your visit, would you be willing to provide some quick feedback?<br /><br />(It will only take a minute)</p><p class='prompt_button'><a href='#' id='oo_launch_prompt'>Yes<span class='screen_reader'>This will open a new window</span></a></p><p class='prompt_button'><a href='#' id='oo_no_thanks'>No Thanks</a></p><p id='ol_brand_logo'><a href='http://www.opinionlab.com/company/'; target='_blank' aria-label='Powered by OpinionLab. This will open a new window'></a></p></div><a id='oo_close_prompt' href='#' aria-label='Close dialog'><div class='screen_reader'>Close dialog</div><span aria-hidden='true'>&#10006;</span></a></div><!--[if IE 8]><style>/* IE 8 does not support box-shadow */#oo_invitation_prompt #oo_invite_content { width: 400px; padding: 40px 49px 20px 49px; border: 1px solid #ccc; }</style><![endif]-->";
    var opt = this.options;
    if(typeof opt.inviteMarkup !== 'undefined') {
      markup = opt.inviteMarkup;
    }
  ...
  function showPrompt(markup, launchFunc) {
    ...
    if (opt.callBacks) {
      ...
      if (typeof opt.callBacks.yesClick === 'function') {
        OOo.addEventListener(promptLauncher, 'click', function () { opt.callBacks.yesClick(); }, false);
      }
      if (typeof opt.callBacks.noClick === 'function') {
        OOo.addEventListener(noThanks, 'click', function () { opt.callBacks.noClick(); }, false);
      }
      if (typeof opt.callBacks.closeClick === 'function') {
        OOo.addEventListener(closePrompt, 'click', function () { opt.callBacks.closeClick(); }, false);
      }
    }

    OOo.addEventListener(promptLauncher, 'click', launchFunc.bind(this), false);
    OOo.addEventListener(noThanks, 'click', function (e) { var evt = e || window.event; OOo.hidePrompt(evt, lastFocused); }, false);
    OOo.addEventListener(closePrompt, 'click', function (e) { var evt = e || window.event; OOo.hidePrompt(evt, lastFocused); }, false);
...
OOo.extend(OOo.Ocode.prototype, {
    tab: function () {
        ...
        New WCAG tab markup
        <a id="oo_tab_1" class="oo_tab_right_1 small" tabindex="1">Feedback<span class="screen_reader">Launch comment card</span></a>
        */
        var d = document,
...
OOo.Invitation.prototype = {
  ...
  launchPopup: function (e) {
    ...
    if (!opt.loadPopupInBackground && OOo.$('oo_container')) { OOo.hidePrompt(evt); }
    ...
    appendWaypoint: function (id) {
        ...
        if (!!el) {
            if (OOo.Browser.isMobile) {
                var touches = 0;
                if ('ontouchstart' in window) {
                    OOo.addEventListener(el, 'touchstart', function (e) {
                        var evt = e || window.event;
                        if (evt.preventDefault && evt.stopPropagation) {
                            evt.preventDefault();
                            evt.stopPropagation();
                    ...
                    OOo.addEventListener(el, 'touchend', function (e) {
                        var evt = e || window.event;
                        if (touches >= 2) {
